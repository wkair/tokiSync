...
## ✨ 주요 기능
  ...
  * **🛡️ 강력한 차단 회피:**
      * **백그라운드 스로틀링 방지:** 탭을 내려놔도 속도가 느려지지 않도록 오디오 트릭(Oscillator)을 사용합니다. (수정중)
      * **자동 재시도:** 네트워크 오류나 구글 429(Too Many Requests) 오류 발생 시 지능적으로 재시도합니다.
      * **캡차 감지:** 진행 중 캡차가 뜨면 일시 정지하고 사용자에게 알림을 보냅니다. (수정중)
...
1.  [구글 스프레드시트](https://sheets.google.com)에 접속하여 **새 스프레드시트**를 생성합니다. (이름은 자유, 예: `TokiDB`)
2.  상단 메뉴에서 **`확장 프로그램`** \> \*\*`Apps Script`\*\*를 클릭합니다.
3.  **`설정(⚙️) 버튼`** \> \*\*`"appsscript.json" 매니페스트 파일 표시`\*\*를 체크합니다.
4.  좌측 파일 목록에서 `appsscript.json`을 클릭하고 아래 내용으로 덮어씁니다.
...
5.  `Code.gs` 파일을 클릭하고 아래 내용으로 덮어씁니다.
      * **`ROOT_FOLDER_ID`**: 만화가 저장될 구글 드라이브 폴더의 ID를 넣으세요.
      * **`SECRET_KEY`**: 원하는 비밀번호를 설정하세요. (클라이언트와 같아야 함)
...
6.  **최초 권한 승인:**
      * 코드 맨 아래에 `function authorizeCheck() { DriveApp.getRootFolder(); }` 같은 임시 함수를 만들고 \*\*실행(▷)\*\*을 누릅니다.
      * 구글 권한 요청 팝업이 뜨면 **[권한 검토]** -\> 계정 선택 -\> **[고급]** -\> **[...로 이동(안전하지 않음)]** -\> \*\*[허용]\*\*을 클릭합니다.
7.  **배포 (Deployment):**
      * 우측 상단 **`배포`** \> **`새 배포`** 클릭.
      * 유형: **웹 앱**.
      * 설명: `TokiSync v1`.
      * 다음 사용자 계정으로 실행: **나 (Me)**.
      * 액세스 권한 승인: **🚨 모든 사용자 (Anyone) 🚨** (매우 중요\!)
...
3.  기존 내용을 지우고 **`TokiSync.user.js`** 전체 코드를 붙여넣습니다.
4.  **`파일`** \> \*\*`저장`\*\*을 누릅니다.
...
3.  아까 복사한 **Apps Script URL**과 설정한 \*\*비밀번호(Key)\*\*를 차례로 입력합니다.
...
화면 우측 하단에 현재 작업 상태가 표시됩니다.
  ...
  * `📦 압축 중`: ZIP/CBZ 파일 생성 중.
  * `🚀 업로드 시작`: 구글 드라이브로 전송 시작.
  * `✅ 완료`: 업로드 및 기록 저장 완료.
  * **리스트 색상:** 작업 중인 항목은 **노란색/파란색**, 완료된 항목은 **회색**으로 변합니다.
